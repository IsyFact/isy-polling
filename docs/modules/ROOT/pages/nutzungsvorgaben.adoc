...
Im xref:konzept.adoc[] werden die konzeptionellen Grundlagen und Vorgaben für Polling definiert, um eine einheitliche Umsetzung in der gesamten xref:glossary::terms-definitions.adoc#glossar-anwendungslandschaft[Anwendungslandschaft] zu gewährleisten.
...
Die Bibliothek `isy-polling` der IsyFact-Standards implementiert den im xref:konzept.adoc[] dargestellten Synchronisationsmechanismus über JMX.
...
image::nutzungsvorgaben/polling-klassendiagramm.png[align="center"]
...
Wird ein Polling-Cluster je zu pollender Datenquelle festgelegt, so kann eine gewisse Lastverteilung erreicht werden.
Das folgende Beispiel zeigt, dass E-Mail-Konten parallel von zwei Instanzen abgearbeitet werden können.
...
Knoten 2 hat das Polling noch nicht begonnen, sodass der Polling-Verwalter im Knoten 1 die Verarbeitung der Nachrichten aus dem E-Mail-Konto 1 erlaubt.
